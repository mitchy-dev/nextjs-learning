...
'TodoItem'
  ...
  test("未完了のタスクをクリック：完了状態になる", async () => {
    const checkIcon = screen.getByLabelText(messages.TodoItem.completeLabel);
  ...
  test("タスクをクリック：編集モードになる", async () => {
   const displayElement = screen.getByLabelText(messages.TodoItem.textLabel);
   const user = userEvent.setup();
  ...
  test("タスクを編集：入力値が反映される", async () => {
    ...
    await user.click(screen.getByLabelText(messages.TodoItem.textLabel));
    ...
    const inputElement = screen.getByLabelText(messages.TodoItem.editLabel);
