...
describe("タスクコンポーネント", () => {
  ...
  test("タスクをクリック：編集モードになる", async () => {
   ...
   expect(mockTodoItemProps.onShowEdit).toHaveBeenCalledWith(mockTodoItemProps.task.id); //呼出チェック、h1とは異なりユーザーイベントが事前に必要
  });
  test("タスクを編集：入力値が反映される", async () => {
    ...
    expect(mockTodoItemProps.onShowEdit).toHaveBeenCalledWith(mockTodoItemProps.task.id);
    ...
    expect(mockTodoItemProps.onChangeText).toHaveBeenCalledWith(mockTodoItemProps.task.id, 'updated');
  });
//   ゴミ箱アイコンクリック：当該要素が消える
  test("ゴミ箱アイコンクリック：当該要素が消える", async () => {
    ...
    expect(mockTodoItemProps.onDeleteTask).toHaveBeenCalledWith(mockTodoItemProps.task.id);
  });
});